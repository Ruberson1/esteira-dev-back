...
class BugImageService
{
    public function uploadImage(Request $request, $bugId)
    {
        ...
        $s3->putObject([
            ...
            'Key' => env('FILE_PATH').$bugId.'/'.$fileName,
        ...
        $bugImage->path = env('FILE_PATH').$bugId.'/'.$fileName;
    ...
    public function updateImage(Request $request, $id)
    {
        ...
        $s3->putObject([
            'Bucket' => env('AWS_NOME_BUCKET'),
            'Key' => env('FILE_PATH').$bugImage->bug_id.'/'.$fileName,
            'Body' => file_get_contents($file),
            'ACL' => env('AWS_PERMISSAO_ENVIO')
        ]);
        $url = $s3->getObjectUrl(env('AWS_NOME_BUCKET'), $fileName);
        $bugImage->path = env('FILE_PATH').$bugImage->bug_id.'/'.$fileName;
        $bugImage->save();
        return [
            'url' => $url,
            'bug_image' => $bugImage
